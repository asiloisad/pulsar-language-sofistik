  ...
  loadMeta() {
    ...
    try {
      if (fs.existsSync(metaPath)) {
        ...
        this.defaultVersion = this._meta.versions[this._meta.versions.length - 1] || this.defaultVersion;
        return this._meta;
      }
    } catch (error) {
  ...
  loadKeywords(version, language) {
    ...
    for (const [moduleName, moduleData] of Object.entries(rawData)) {
      ...
      for (const [cmdName, paramsList] of Object.entries(moduleData)) {
        result[moduleName][cmdName] = Array.isArray(paramsList) && paramsList.length > 0
