...
class SofistikKeywordsProvider {
  constructor() {
    // Cache for all loaded keywords: { "2026.en": {...}, "2025.de": {...} }
    this.cache = {};
  ...
  /**
   * Load keywords from version and language specific JSON file (with caching)
  ...
  loadKeywords(version, language) {
    ...
    try {
      if (fs.existsSync(keywordsPath)) {
        ...
        this.cache[cacheKey] = JSON.parse(data);
        return this.cache[cacheKey];
      } else {
        // Fall back to default version if version not found
        console.warn(
          `SOFiSTiK keywords not found for version ${version}, falling back to ${this.defaultVersion}`
        );
        if (version !== this.defaultVersion) {
          return this.loadKeywords(this.defaultVersion, language);
        } else {
          this.cache[cacheKey] = {};
          return this.cache[cacheKey];
        }
  ...
  getAvailableVersions() {
    ...
    try {
      ...
      for (const file of files) {
        const match = file.match(/^sofistik\.(\d{4})\.en\.json$/);
        if (match) {
